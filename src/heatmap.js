(function($) {
  ...
  function update_legend(legendContainer, options) {
    ...
    if (options.format_str) format_str = options.format_str;
      ...
      format_str = domain[0] % 1 === 0 && domain[1] % 1 === 0 ? ",.0f" : "0.2f";
  ...
  var methods = {
    init: function(option, callback) {
      ...
      var defaults = {
        ...
        on_click: null,
        eachfeature: function(p, x, l) {
          l.bindTooltip(x.name);
        },
        show_legend: true,
        max_width: null,
        save_button: true,
      ...
      function default_geodata_parser(loaded) {
        ...
        var geojson = topojson.feature(
          loaded,
          loaded.objects[geodata_fieldname]
        );
          ...
          onAdd: function(map) {
            ...
            captionContainer
              ...
              .style("font-size", function(d, i) {
                return options.caption_sizes[i] + "pt";
              })
              .text(function(d) {
                return options[d];
              });
        ...
        if (options.save_button) {
          var saveButton = L.Control.extend({
            ...
            onAdd: function(map) {
              ...
              var saveButtonElement = L.DomUtil.create(
                "a",
                "",
                buttonContainer
              );
              ...
              L.DomEvent.addListener(buttonContainer, "click", function() {
                html2canvas(document.getElementById("main_view"), {
                  onrendered: function(canvas) {
                    var svg_data = new XMLSerializer().serializeToString(
                      $("div.leaflet-overlay-pane svg")[0]
                    );
                    var img =
    ...
    get_commune_def: function() {
      return {
        communes: $(this.selector)[0].japaneseMapCommunes,
        id_map: $(this.selector)[0].japaneseMapIdMap
      };
    }
  };
